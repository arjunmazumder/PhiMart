...
class SimplifyAddCartItemSerializer(serializers.ModelSerializer):
    ...
    def validate_product_id(self, product_id):
        if not Product.objects.filter(pk=product_id).exists():
            raise serializers.ValidationError('product id does not exists')
        return product_id
...
class CartSerializer(serializers.ModelSerializer):
    items = CartItemSetializer(many=True, read_only = True)
    total_cart_price = serializers.SerializerMethodField(method_name='get_total_price')
    class Meta:
        model = Cart
